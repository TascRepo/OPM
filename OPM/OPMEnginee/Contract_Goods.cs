...
namespace OPM.OPMEnginee
{
    public class Contract_Goods
    {
        ...
        public Contract_Goods(string idContract, string name, string origin, string manufacturer = "VNPT Technology", string code = "iGate GW020-H", string unit = "Bộ", double priceUnit = 0, int quantity = 0, string note = "", string license = "Bản quyền ONT VNPT Techlonogy", string name1 = "Thiết bị đầu cuối ONT loại  (2FE/GE + Wifi Dualband)")
        {
        ...
        public double GetPriceUnit(string IdContract)
        {
            string query = string.Format("SELECT top 1 priceUnit FROM dbo.Contract_Goods WHERE idContract = '{0}'", IdContract);
            DataTable table = OPMDBHandler.ExecuteQuery(query);
            if (table.Rows.Count > 0)
            {
