...
namespace OPM.OPMEnginee
{
    public class ContractObj
    {
        ...
        public ContractObj(DataRow row)
        {
            Id = row["id"].ToString();
        ...
        public static List<ContractObj> GetList()
        {
            List<ContractObj> contracts = new List<ContractObj>();
            string query = string.Format("SELECT * FROM dbo.Contract");
            DataTable dataTable = OPMDBHandler.ExecuteQuery(query);
            foreach (DataRow row in dataTable.Rows)
            {
                ContractObj contract = new ContractObj(row);
